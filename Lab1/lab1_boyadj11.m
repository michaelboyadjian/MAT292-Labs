...
% work as:
%
% Error for negative index: "Index exceeds the number of array elements(6)"
%
% Error for zero index: "Array indices must be positive integers or logical
% values"

%
% Error for positive index outside of the bounds: "Array indices must be
...
N = 101; % need 101 intervals     
h = (7-(-3))/(N-1);
x = zeros(N,1); % create N element long vector of zeros
...
y = zeros(N,1); % create N element long vector of zeros
...
% create 5000001 element long vector
N = 5000001;             
h = (3-(-2))/(N-1);
x = zeros(N,1);     
...
% Description
%
% The general solution to the ODE is y(t) = (t+C)/(exp(t^2))
% The particular solutions for the 3 given initial value problems are as
% follows:
%   y(1) = -1    ->     y(t) = (t-e-1)/(e^(t^2))
%   y(1) = 0     ->     y(t) = (t-1)/(e^(t^2))
%   y(1) = 1     ->     y(t) = (t+e-1)/(e^(t^2))

clear all;

% create vector with 200 intervals b/w -10 and 10
...
xlabel("t");
ylabel("y");
title("Solution to a First-Order ODE");
hold on;
plot (t,y_3);
legend('y_1','y_2','y_3');
...
% Array from -5 to 5 with 100 points
x = linspace(-5,5,100);
plot(x, f(x));
% Label axes
xlabel('x');
...
for j = 1:length(x)
    guess = exp(-sin(x(j)+x(j).^3));
    y(j) = fzero(@(y)f(x(j),y),guess);
end
...
% As x approaches inifinity, the positive y solution has a wider parabola
% and takes longer to reach infinity. The negative y solution becomes more
% parabolic, but continues to approach 0 at the same rate as other
% solutions. The solutions are symmetric about the y-axis since y(x) =
% y(-x) as stated. Visually speaking, this means that the positive and
% negative sides of the graph of solutions are reflections of each other in
% the y-axis.
...
% For values of y < -1, the solution approaches -1 as x approaches
% infinity. For -1 < y < 1, y approaches 1 as x approaches infinity. For 
% 1 < y < 2, approaches 1 as x approaches infinity. And finally, for y > 2,
% y approaches infinity as x approaches infinity.
