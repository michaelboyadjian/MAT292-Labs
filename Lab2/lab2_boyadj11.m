...
% Compare to exact solution y(t) = (sin^2(t) - 1)/(2cos(t))
tt = linspace(0,3.2,50);
yy = sec(tt) .*(sin(tt).^2 ./ 2 -0.5);
...
% Exact Solution
yexact = sec(soln.x) .* (sin(soln.x).^2 ./2 -0.5);
...
% The largest error occurs arounf the t = 1.6 mark. Here there is a cusp in
% the graph of the exact solution. This means that y' is changing signs
% nearly instantaneously. With this change in the sign of the
% function, there is less accuracy when using the numerical methods,
...
% Inline Function + Solution to ODE
f = @(t, y) 1 ./ y^2;
t0 = 1;
y0 = 1;
t1 = 10;
...
% Set Initial Approximation = 1
phi=sym(1);   

% For Loop for Picard Iterations
N=5;
for i = 1:N
    func=subs(f,y,phi+1);  % Prepare for integration    
    func=subs(func,t,s);   % Change variable of integration from s to t 
    newphi = int(func, s, 0 ,t);  % Integrate for next approximation
    phi=newphi;  % Update phi to newphi
end
...
eval(subs(phi, t, 0));
set(picard,'Color','green');    % Set the colour of the graph to green
 
% Exact Solution -> y = tan(t + pi/4)
